...
streaks AS (
    SELECT *,
           ...
           CASE WHEN round_ > 5 THEN SUM(points) OVER (PARTITION BY team_id ORDER BY fixture_date ROWS BETWEEN 5 PRECEDING AND 1 PRECEDING) END AS last_5_matches,
           CASE WHEN round_ > 10 THEN SUM(points) OVER (PARTITION BY team_id ORDER BY fixture_date ROWS BETWEEN 10 PRECEDING AND 1 PRECEDING) END AS last_10_matches,
           CASE WHEN round_ > 3 THEN SUM(points) OVER (PARTITION BY team_id ORDER BY fixture_date ROWS BETWEEN 3 PRECEDING AND 1 PRECEDING) END AS last_3_matches,
